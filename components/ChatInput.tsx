...
export default function ChatInput({ onSend, isLoading, onStop, webSearch, onToggleWebSearch, editingMsg, onCancelEdit, onEditSend }: ChatInputProps) {
  ...
  useEffect(() => {
    if (editingMsg) {
      ...
      setAttachments(editingMsg.attachments || []);
  ...
  const handleSend = () => {
    ...
    if (editingMsg) {
      onEditSend?.(trimmed || "Please analyze the attached files.", attachments.length > 0 ? attachments : undefined);
    } else {
  ...
  return (
    <div className="border-t border-[#3b3558] bg-[#13111c]/80 backdrop-blur-sm p-2 sm:p-4">
      <div className="max-w-4xl mx-auto">
        {editingMsg && (
          <div className="flex items-center justify-between mb-2 px-3 py-2 bg-[#7c3aed]/10 border border-[#7c3aed]/30 rounded-xl">
            <span className="text-xs text-[#a78bfa] flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
              Editing message — the AI response will be regenerated
            </span>
            <button
              onClick={() => onCancelEdit?.()}
              className="text-xs text-gray-400 hover:text-white px-2 py-1 rounded hover:bg-[#3d3760] transition-all"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="flex items-end gap-1.5 sm:gap-3 bg-[#1e1b2e] border border-[#3b3558] rounded-2xl px-2.5 sm:px-4 py-2 sm:py-3 focus-within:border-[#7c3aed] transition-colors">
          ...
          <button
            ...
            title={isLoading ? "Stop generating" : editingMsg ? "Save edit" : "Send"}
          >
            {isLoading ? (
